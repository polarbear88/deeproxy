...
### 七·补 — fake-ip 占位地址显示原理（198.18.0.0/15，非 bug）

部分客户端（Clash / Surge / sing-box 等）启用 **fake-ip 模式**时，会在本地 DNS 层为域名分配一个**占位 IP**（通常落在 `198.18.0.0/15` 这个保留段，如 `198.18.1.255`），再把这个占位 IP 作为 SOCKS5 CONNECT 的目标地址发给本服务——**此时 CONNECT 请求里根本没有域名（FQDN 为空）**，真实域名只存在于客户端本地的 fake-ip 映射表中，服务端无从知晓。

因此会出现如下**预期行为**（非服务端解析错误）：

- 当 CONNECT 目标是 `198.18.x.x` 段 IP、且首包**域名嗅探**（TLS SNI / HTTP Host）**失败 / 超时 / 为非 TLS-HTTP 流量**时，「实时连接」列表与系统日志会**如实显示该占位 IP**（如 `198.18.1.255`），而不是域名。
- 原因：服务端 `targetHost()` 读取的就是 CONNECT 请求里的原始目标地址；只有当嗅探成功还原出域名时，才会 `SetTarget` 把目标覆盖为域名。嗅探不成功就没有域名可显示，占位 IP 即为目标地址的真实样貌。
- 这不是 bug，也不影响转发：占位 IP 仍会按 ip-cidr 规则 / 默认动作正常选路转发到上游，由上游侧完成真正出口。

**推荐做法**：客户端改用**远程 DNS（`socks5h://` / 「让代理解析域名」）**，由客户端直接把**域名**放进 CONNECT 请求发来。这样路径最短、无需嗅探、规则按域名精确命中，且即便命中 `reject` 也能回标准 SOCKS5 拒绝码（嗅探路径因需先回 success 才能读首包，命中 reject 只能表现为关闭连接）。
